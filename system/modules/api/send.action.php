...
class send extends SystemAction {
    ...
    */
    public function send_shop_code(){
        ...
        // jasper 增加查询订单号
        $orderInfo = $db->GetOne("SELECT * FROM `@#_member_go_record` WHERE `shopid` = '$gid' and `uid` = '$uid'");
        ...
        if($type=='3'){
            // jasper 修改为发送订单号
            if(!empty($member['email'])){
                send_email_code($member['email'],$username,$uid,$orderInfo['code'],$info['title']);
                $ret_send = true;
            }
            if(!empty($member['mobile'])){
                send_mobile_shop_code($member['mobile'],$uid,$orderInfo['code']);
                $ret_send = true;
            }
        }
        //只发送微信
    ...
    public function sendMessage(){
        ...
        $uid=$this->segment(5);
        $code=$this->segment(6);
        $valid=$this->segment(7);
        var_dump("success");
        if($valid=="token")
        {
            send_mobile_shop($mobile,$uid,$code);
        }
    }
}