...
$shoparr = $this->db->GetList ( "SELECT * FROM `@#_shoplist` where `id` in($shopids)", array ("key" => "id" ) );}
 ...
 //修改支付每次都要使用福分问题 lq 2014-12-01
		...
		if($ok != 'ok'){
		  ...
		  if ($check) {
			  // 成功
			  header ( "location: " . WEB_PATH . "/mobile/cart/payspaysuccessuccess" );
		  } else {
			  // 失败
			  _setcookie ( 'Cartlist', NULL );
	...
	public function jf_paysubmit(){
		...
		if($ok != 'ok'){
			...
			_setcookie("Cartlist_jf",null);
			_messagemobile("购物车没有商品请<a href='".WEB_PATH."/mobile/cart/jf_cartlist' style='color:#C40000'>返回购物车</a>查看");
		}

		$check = $pay->jf_go_pay($pay_checkbox);
		if($check + 100 == 0){
			_messagemobile("账户余额不足以支付，请<a href='".WEB_PATH."/mobile/home/userrecharge' style='color:#C40000'>充值</a>");
		}
		if(!$check){
			_messagemobile("订单添加失败,请<a href='".WEB_PATH."/mobile/cart/jf_cartlist' style='color:#C40000'>返回购物车</a>查看");
		}
		if($check){
			//成功
