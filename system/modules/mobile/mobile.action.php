...
class mobile extends base {
	...
	public function init(){
		...
		//最新揭晓
		$shopqishu=$this->db->GetList("select * from `@#_shoplist` where `q_end_time` !='' ORDER BY `q_end_time` DESC LIMIT 6");
	...
	public function glist(){
		...
		$orders =htmlspecialchars($this->segment(4));
		//var_dump($orders);
		if($orders == 's10'){
			$onesa = '即将揭晓';
		}elseif ($orders == 's20') {
		...
		if($cate_band > 0){
			...
			$daohang=$this->db->GetOne("select * from `@#_category` where `cateid` = '$cate_band' order by `order` DESC");
			$key = $daohang['name'];
		}else{
			$key="全部商品";
		}
...
//ajax获取商品列表信息
	...
	public function itemajax(){
        ...
        echo '<div class="m-lott-pic">   <a href="'.WEB_PATH.'/mobile/mobile/item/'.$itemid.'" title="'.WEB_PATH.'/mobile/mobile/itemajax/'.$itemid.'"> <img src="'.$temp['pic'].'" border="0" alt="'.$temp['title'].'"></a></div><p>中奖</p><div class="m-lott-state"><span style="color: #5bb6ea;">'.$temp['user'].'</span></div>';exit;
	}
    ...
    public function movements(){
        ...
        $newid = $this->db->GetOne("select id from `@#_shoplist` where `sid`='$item[sid]' order by `qishu` DESC LIMIT 1");
        $itemlist = $this->db->GetList("select * from `@#_shoplist` where `sid`='$item[sid]' AND `q_user_code` is not null order by `qishu` DESC LIMIT 10");
        $shoplist=$this->db->GetList("SELECT gonumber,huode,shopqishu,shopid FROM `@#_member_go_record` WHERE `shopname`='$item[title]' ORDER BY shopqishu desc");

        foreach ($itemlist as $k => $value) {
            $userinfo=unserialize($value['q_user']);
            if($userinfo['username']){
                if(is_numeric($userinfo['username'])){
                    $itemlist[$k]['q_user']=substr_replace($userinfo['username'], '****', 3, 4);
                }else{
                    $itemlist[$k]['q_user']=$userinfo['username'];
                }
            }else{
                $itemlist[$k]['q_user']=substr_replace($userinfo['mobile'], '****', 3, 4);
            }
            $num=$this->db->GetOne("select sum(gonumber) as gnum from `@#_member_go_record` where shopid=".$value['id']." AND uid=".$value['q_uid']);
            $itemlist[$k]['buycount']=$num['gnum'];
            $shoplist=$this->db->GetList("SELECT gonumber,huode,shopqishu,shopid FROM `@#_member_go_record` WHERE `shopname`='$item[title]' AND `shopqishu` = '$value[qishu]' ");

            foreach ($shoplist as $ky => $v) {
                $itemlist[$k]['goucount']+=$v['gonumber'];
                if($v['huode'] == $value['q_user_code'] && $v['qishu'] == $value['shopqishu']){
                    break;
                }
        ...
        include templates("mobile/index","movements");
    }
}
?>