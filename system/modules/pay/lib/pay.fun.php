	...
	if($u_info){
		...
		if($q){
			...
			if(!$q) {
				...
				// 修改中奖发送方式 Jasper 20181203
                $send = System::load_sys_config("send");

				$post_arr= array("uid"=>$u_info['uid'],"gid"=>$shop['id'],"send"=>$send);
...
function pay_go_yongjin($uid=null,$dingdancode=null){
	...
	$config = System::load_app_config("user_fufen",'','member');//����/����/Ӷ��

	$yesyaoqing=$db->GetOne("SELECT * FROM `@#_member` WHERE `uid`='$uid'");

	if($yesyaoqing['yaoqing']){

		$yongjin=$config['fufen_yongjin']; //ÿһԪ���ص�Ӷ��
...
//	var_dump($yesyaoqing);exit;
	...
	foreach($gorecode as $val){
		...
		$content="(��".$val['shopqishu']."��)".$val['shopname'];
        foreach ($yqs as $k=>$v ) {
			$s = $k+1;
			$t = explode('|',$v);
