...
class sendmobile {
	...
	**/	
	...
	/*郑州商讯短信其他操作*/
//铭翼短信平台短信发送
	private function cfg_seting_9(){
		return true;
	}
	private function cfg_send_9(){
		$mobile = $this->mobile['cfg_mobile_9'];	
		$name = urlencode($mobile['mid']);
		$pwd  = $mobile['mpass'];
		$haoma = $this->config['mobile'];
		...
		$post_data['secret'] = $pwd;
		$post_data['mobile'] = $haoma;
		$post_data['body'] = '【'.$mobile['mqianming'].'】'.$this->config['content'];
		
		$url='http://pay.mingyie.com/api/sms';
	...
	public function cfg_getdata_9(){
		...
		$md5secret = md5($mobile['mid'].$mobile['mpass']);
		$url = 'http://pay.mingyie.com/api/chaxun/sms';
		$param = array(
		'user_id'=>$mobile['mid'],
		'md5secret'=>$md5secret
		);
		
		$json = $this->request_post($url,$param);
		$rest = json_decode($json,true);
