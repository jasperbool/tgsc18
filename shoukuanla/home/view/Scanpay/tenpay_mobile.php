...
if($post['isWriteNote'] == '1'){
   ...
   $tenqrcodePath=SKL_WEBROOT_PATH.'images/tenqrcode.jpg';
   echo '
   <p><b>第二步：输入指定金额后付款，注意必须需输入指定金额否则会充值失败</b></p>  ';

}else{ 	      
  
   $tenqrcodePath=SKL_WEBROOT_PATH.'images/tenqrcode/'.$post['money'].'/'.$post['fileName'];

}

echo '<p><b>指定金额：<a style="color: #F00;font-size: 30px;">'.$post['titleShort'].'</a></b></p>
  <p><img id="Qrcode" src="'.$tenqrcodePath.'?cacheTime='.$cacheTime.'" width="70%" /></p>
';	
...
<div class="copy">
<form name="form">
 <div class="inputEmail"> <input onClick="this.select()" name="Email" class="inputsty" name="input" type="text" value="<?php echo $post['email']; ?>" readonly> </div>

 <div class="inputEmail" style="padding-top:8px;"> <input onClick="this.select()" style="width: 170px; color: #f6a708;" class="inputsty" name="money" type="text" value="<?php echo $post['titleShort']; ?>" readonly> </div>
 
 <div class="inputEmail" style="padding-top:43px;"> <input onClick="this.select()" style="width: 170px;" class="inputsty" name="text" type="text" value="" readonly> </div>
</div>
</p>
 ...
 $.get("<?php echo $this->cfg_findOrderUrl; ?>", { order: order,timestamp:new Date().getTime() },
  function(data){   
     ...
     if(data.<?php echo $this->cfg_stateField; ?> == '<?php echo $this->cfg_stateValue; ?>'){	
	    ...
	    $("#Qrcode").attr("src","<?php echo SKL_WEBROOT_PATH; ?>images/failure.png"); 
	    $("#urlPay").attr("href","<?php echo $post['returnUrl']; ?>");
	    window.clearInterval(timerQuerystatus);//停止ajax
	  }
  },'json');
...
function jump(){
  ...
  window.location.href='<?php echo $post['returnUrl']; ?>';
}
