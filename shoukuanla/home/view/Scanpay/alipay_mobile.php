...
if($post['isWriteNote'] == '1'){
   ...
   $aliqrcodePath=SKL_WEBROOT_PATH.'images/aliqrcode.jpg';
   echo '
   <p><b>第二步：输入指定金额后付款，注意必须需输入指定金额否则会充值失败</b></p>  ';

}else{ 	      
  
   $aliqrcodePath=SKL_WEBROOT_PATH.'images/aliqrcode/'.$post['money'].'/'.$post['fileName'];

}

echo '<p><b>指定金额：<a style="color: #F00;font-size: 30px;">'.$post['titleShort'].'</a></b></p>
  <p><img id="Qrcode" src="'.$aliqrcodePath.'?cacheTime='.$cacheTime.'" width="70%" /></p>
';	
...
<form name="form">
 ...
 <div class="inputEmail" style="padding-top:60px;"> <input style="font-weight:bold;" onClick="this.select()" class="inputsty" name="money" type="text" value="<?php echo $post['titleShort']; ?>" readonly> </div>
